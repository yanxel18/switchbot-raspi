...
def connect(device: str, bt_interface: str, timeout: float):
    ...
    while not req.is_connected():
        if time.time() - connect_start_time >= timeout:
            raise ConnectionError('Connection to {} TIMEDOUT after {} seconds'.
...
def main():
    ...
    parser.add_argument('--connect-timeout', dest='connect_timeout', type=int, required=False, default=6,
    ...
    print('Command execution successful;Complete')
